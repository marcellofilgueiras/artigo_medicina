...
#Número de Mortes por Trem Gástrico por mês
...
#número de Mortes por Hérnia por mês
...
#número de Mortes por Hérnia por mês em Linha
...
sim_join_2018_2020 %>%
  ...
  count(causa_basica)%>%
  tsibble::as_tsibble(index=mes, key = causa_basica) %>%
  ggplot(aes(x=mes, y=n, colour=causa_basica, group= causa_basica )) + 
  geom_smooth(size=1) +
  labs(title= "Número de Mortes por Hérnia por mês",
       subtitle = "Dados de 2018 - 2020",
       caption = "Fonte: Sistema Informatizado de Mortalidade (SIM) - Open Data Sus e Base dos Dados") +
